...
MQTT_BROKER_HOST="a3kbz4r8n71kk7-ats.iot.us-west-2.amazonaws.com"
...
GenerateFile()
{
    ...
    privKey=$(cat privKey)
    csv_file="$(pwd)/kasbraun.csv"
    OLDIFS=$IFS
    [ ! -f $csv_file ] && { echo "$csv_file file not found"; exit 99; }
    # Cycle among the csv rows and extract the 3 columns
    while IFS=, read -r latitude longitude
    do
        # Do some work with columns values
        # GPS=(3158.46080 11848.37307)
        longitude=$(echo $longitude|awk -F "\r" '{print $1 }')
        GPS[0]=$latitude
        GPS[1]=$longitude
        let timestp=timestp+5000
        randomHex=$(openssl rand -hex 8)
  ...
  #   do
	# NextData
        # objMessage="\"message\":{\"snr\":$SNR,\"vbat\":$VBAT,\"latitude\":${GPS[0]},\"longitude\":${GPS[1]},\"gasResistance\":${ENV[0]},\"temperature\":${ENV[1]},\"pressure\":${ENV[2]},\"humidity\":${ENV[3]},\"timestamp\":\"$timestp\",\"random\":\"$randomHex\"}"
        ...
        objMessage="\"message\":{\"accelerometer\":[${accel[0]},${accel[1]},${accel[2]}],\"latitude\":${GPS[0]},\"longitude\":${GPS[1]},\"timestamp\":\"$timestp\",\"random\":\"$randomHex\"}"
        sign_msg="{$objMessage}"
...
AWSIOTUpload()
{
    ...
    do
        ...
        #mosquitto_pub -t  $default_pubtopic -m $objMsg -h $MQTT_BROKER_HOST  --cafile "$(pwd)/AmazonRootCA1.pem" --cert  "$(pwd)/70271fb0b1-certificate.pem.crt" --key  "$(pwd)/70271fb0b1-private.pem.key"  --insecure -p $MQTT_BROKER_PORT
        mosquitto_pub -t  $default_pubtopic -m $oneline -h $MQTT_BROKER_HOST  --cafile "$(pwd)/AmazonRootCA1.pem" --cert  "$(pwd)/70271fb0b1-certificate.pem.crt" --key  "$(pwd)/70271fb0b1-private.pem.key"  --insecure -p $MQTT_BROKER_PORT
...
PebbleRegistration()
{
    ...
    echo ""
    regRq=$(mosquitto_sub  -t  "device/${device_id}/action/add" -h $MQTT_BROKER_HOST -C 1 --cafile "$(pwd)/AmazonRootCA1.pem" --cert  "$(pwd)/70271fb0b1-certificate.pem.crt" --key  "$(pwd)/70271fb0b1-private.pem.key"  --insecure -p $MQTT_BROKER_PORT)
    ...
    mosquitto_pub -t  "device/${device_id}/action/confirm" -m "$msg" -h $MQTT_BROKER_HOST  --cafile "$(pwd)/AmazonRootCA1.pem" --cert  "$(pwd)/70271fb0b1-certificate.pem.crt" --key  "$(pwd)/70271fb0b1-private.pem.key"  --insecure -p $MQTT_BROKER_PORT
    echo ""
    echo  "Succesfully published!"
    echo ""
    echo "Press any key to return to the main menu"
...
upload_config()
{
    ...
    mosquitto_pub -t  "device/${device_id}/config" -m "${confStr}" -h $MQTT_BROKER_HOST  --cafile "$(pwd)/AmazonRootCA1.pem" --cert  "$(pwd)/70271fb0b1-certificate.pem.crt" --key  "$(pwd)/70271fb0b1-private.pem.key"  --insecure -p $MQTT_BROKER_PORT
}
isInstalled()
{
    toolpath=$(which $1)
